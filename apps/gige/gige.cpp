...
void Run(const std::string& camera_name, bool display, size_t frames_per_file) {
  ...
  if (display) {
    ...
    while (true) {
      ...
      AddText(
          image_to_show,
          std::string() + "[E] Exposure: " + std::to_string(camera->GetExposure()),
          row_idx++);
      AddText(
          image_to_show,
          std::string() + "[N] Gain: " + std::to_string(camera->GetGain()) + "dB",
          row_idx++);
      AddText(image_to_show, "--------------------", row_idx++);
      AddText(
          image_to_show,
          std::string() + "[S] Sharpness: " + std::to_string(camera->GetSharpness()),
          row_idx++);

      AddText(
          image_to_show,
          std::string() + "[V] Hue: " + std::to_string(camera->GetHue()) + " deg",
          row_idx++);
      AddText(image_to_show,
