...
// The reason we take a mat as a parameter is to avoid dealing with figuring out
// the correct size for the vishash
void FramePush(cv::Mat m) {
  ...
  while (true) {
    ...
    frame->SetValue("Camera.Benchmark.StartTime",
...
void Run(const std::vector<string>& camera_names, const string& model_name,
  ...
  if (!use_fake_nne) {
    ...
    imagematch->SetSource("input",
                          neural_net_eval->GetSink(vishash_layer_name));
...
int main(int argc, char* argv[]) {
  ...
  desc.add_options()("use_fake_nne,u",
                     "use fake NNE for max throughput or not");
  ...
  desc.add_options()("fast_query_population,f", "Use fast query population");
  po::variables_map vm;
  try {
