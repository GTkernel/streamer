...
void Run(const std::string& publish_endpoint, const std::string& camera_name, 
  ...
  sender->SetSource(camera->GetStream());
  ...
  std::cout << "real sending latency = " << sender->send_latency_ms_sum / frame_count << std::endl;
}
...
int main(int argc, char* argv[]) {
  ...
  Run(publish_endpoint, camera_name, exec_sec);
  return 0;
}
