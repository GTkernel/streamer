...
def plot_tsne_with_keyframe_train_windows(
  ...
  def get_min_dimension_width(embedded_data):
    xlen = max(embedded_data[:, 0]) - min(embedded_data[:, 0])
    ylen = max(embedded_data[:, 1]) - min(embedded_data[:, 1])
    return min(xlen, ylen)

  gs2 = gridspec.GridSpec(2, 2)
  gs2.update(left=0.46, right=1.0, hspace=0.02, wspace=0.02)
  grid_elems = [gs2[row, col] for row in range(0, 2) for col in range(0, 2)]
  ...
  for i, train_frame in enumerate(train_frames):
    ...
    text_offset_width = 0.02 * get_min_dimension_width(embedded_data)
    ax1.text(min_x - text_offset_width, min_y + rect_height - text_offset_width,
             str(i + 1), fontsize=14)
    ...
    # HACK: Plot invisible bounding box points on global plot to automatically
    #   scale axes appropriately to include boxes.
    ax1.scatter([min_x, min_x, min_x + rect_width, min_x + rect_width],
                [min_y, min_y + rect_height, min_y, min_y + rect_height],
                alpha=0)
