...
def __parse_args():
    ...
    parser.add_argument(
        ...
        "--context-frames",
        default=60,
        help="The number of frames before and after each keyframe to include in the context.",
        required=False,
        type=int)
    parser.add_argument(
        "-s",
    ...
    if not context_frames > 0:
        raise Exception(
            "--context-frames must be greater than 0, but is: {}".format(context_frames))
    sel = args.sel
    if sel <= 0 or sel > 1:
...
def __extract_segment(keyframe, total_frames, context_frames, segment_dir, input_filepath):
    """Extracts the video segment surrounding the provided keyframe

    Returns the filepath of the video segment.
    """
    dur_f = context_frames * 2
    start_f = dur_f / 2
...
def main():
    ...
    sel = args.sel
    buf_len = args.buf_len
    kd_cmd = ("{} --save-jpegs --block ".format(kd_app) +
    ...
    for k in k_idxs:
        __extract_segment(k, total_frames, args.context_frames, segment_dir, input_filepath)
