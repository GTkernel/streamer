...
inline static half* load_image(const cv::Mat& image, const cv::Size& size, const float* mean, const float* std) {
    float *data32;
    half *data16;
    cv::Mat image32;
    ...
    floattofp16((unsigned char *) data16, data32, image32.total() * image32.channels());
...
bool NCSManager::IsOpened() const {
    return GetNumDevices() > 0;
}

int NCSManager::GetNumDevices() const {
    return _names.size();
}
