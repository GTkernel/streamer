...
MXNetClassifier::MXNetClassifier(const string &model_desc,
  ...
  const mx_uint input_shape_data[4] = {1, static_cast<mx_uint>(3),
                                       static_cast<mx_uint>(input_width),
                                       static_cast<mx_uint>(input_height)};

  MXPredCreate((const char *)json_data.GetBuffer(),
               (const char *)param_data.GetBuffer(),
               static_cast<size_t>(param_data.GetSize()), dev_type, dev_id,
...
std::vector<float> MXNetClassifier::Predict() {
  ...
  // Get Output Result
  MXPredGetOutputShape(predictor_, output_index, &output_shape,
                       &output_shape_len);

  size_t output_size = 1;
  for (mx_uint i = 0; i < output_shape_len; ++i) output_size *= output_shape[i];
...
DataBuffer MXNetClassifier::GetInputBuffer() { return input_buffer_; }
