...
NNBench::NNBench(
    const ModelDesc& model_desc, const Shape& input_shape, size_t batch_size, int num_classifiers, bool run)
...
std::shared_ptr<NNBench> NNBench::Create(
...
void NNBench::SetSource(StreamPtr stream,
                                   const std::string& layername) {
  SetSource(SOURCE_NAME, stream, layername);
}

StreamPtr NNBench::GetSink() {
...
void NNBench::Process() {
  ...
  if(run_) {
    for(int i = 0; i < classifiers_; ++i) {
      model_->Evaluate({{input_layer_name_, cur_batch_}},
                                            {LAYER}, nullptr);
    }
  }
  long time_elapsed =
