...
TrainDetector::TrainDetector()
    : Processor(PROCESSOR_TYPE_THROTTLER, {SOURCE_NAME}, {"output"}),
      num_divid(10),
      display_scalar(1),
      hasTrain(false) {}
...
bool TrainDetector::Init() { 
  ...
  cv::Ptr<cv::BackgroundSubtractor> pMOG=cv::createBackgroundSubtractorMOG2(1000, 16, false);
  hasTrain = false;
...
void TrainDetector::Process() {
  ...
  const cv::Mat& img_input = frame->GetValue<cv::Mat>("image"); //original_image
  cv::Mat img_mask;
  ...
  for (int i = 0; i < num_divid; i++){
    ...
    cv::Mat stripe_mask(img_input.rows, img_input.cols, CV_8UC1, cv::Scalar(0, 0, 0));
    cv::Mat stripe_roi = RoI_mask(cv::Rect(width*i, 0, width, img_input.rows));
    stripe_roi.cv::Mat::copyTo(stripe_mask(cv::Rect(width*i, 0, width, img_input.rows)));
