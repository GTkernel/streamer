...
void ImageSegmenter::Process() {
  // Do image segmentation
    ...
    cv::Mat output_score = cv::Mat::zeros(output.size[1], output.size[0],
    CV_32F); for (const auto& channel : output_split) { CHECK(channel.dims ==
    output_img.dims); CHECK(channel.size[0] == output_img.size[0]);
