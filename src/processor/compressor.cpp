...
Compressor::Compressor(Compressor::CompressionType t, int num_threads) :
    Processor(PROCESSOR_TYPE_CUSTOM, {"input"}, {}), compression_type_(t),
    ioService_(), threadpool_(), work_(ioService_) {
...
void Compressor::Process() {
  ...
  std::shared_ptr<LockedFrame> locked_frame = std::make_shared<LockedFrame>(std::move(frame));
  {
