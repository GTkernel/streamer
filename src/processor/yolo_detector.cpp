...
cv::Mat Detector::Detect(const cv::Mat& img) {
  ...
  cv::Mat FinalImage =
      GetBox(DetectionResult, &pro_obj[0][0], idx_class, bboxs, 0.2, img);
  return FinalImage;
}
    ...
    for (int j = 0; j < 7; ++j) {
      for (int k = 0; k < 2; ++k) {
        if (pro_obj[(i * 7 + j) * 2 + k] > thresh) {
          // std::cout << "(" << i << "," << j << "," << k << ")" << "
          // prob="<<pro_obj[(i*7+j)*2 + k] << "
          // class="<<idx_class[i*7+j]<<std::endl;
...
YoloDetector::YoloDetector(const ModelDesc& model_desc, float idle_duration)
