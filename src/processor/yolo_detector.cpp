...
void YoloDetector::Process() {
  ...
  auto frame = GetFrame("input");
    ...
    auto image = frame->GetValue<cv::Mat>("image");
		...
		std::vector<std::vector<int>> bboxes=GetBoxes(DetectionOutput,&pro_obj[0][0],idx_class,bboxs,confidence_threshold_,image);
    ...
    auto original_img = frame->GetValue<cv::Mat>("original_image");
		...
		for (size_t i = 0; i < filtered_res.size(); ++i) {
          ...
          bbs.push_back(Rect(filtered_res[i][1], filtered_res[i][2], filtered_res[i][3]-filtered_res[i][1], filtered_res[i][4]-filtered_res[i][2]));
