...
inline static float box_intersection(const cv::Rect_<float>& a, const cv::Rect_<float>& b)
{
...
inline static float box_union(const cv::Rect_<float>& a, const cv::Rect_<float>& b)
{
...
inline static float box_iou(const cv::Rect_<float>& a, const cv::Rect_<float>& b)
{
...
inline static void get_boxes(std::vector<std::vector<float>>& probs, std::vector<cv::Rect_<float>>& boxes, const std::vector<float>& predictions, int classes, bool only_objectness=false) {
    float thresh = 0.1f;
...
inline static void nms_sort(std::vector<std::vector<float>>& probs, const std::vector<cv::Rect_<float>>& boxes, int classes) {
	float thresh = 0.35f;
...
inline static void get_detections(std::vector<std::tuple<int, cv::Rect, float>>& detections, const cv::Size& size, const std::vector<std::vector<float>>& probs, const std::vector<cv::Rect_<float>>& boxes, int classes) {
    float thresh = 0.1f;
    ...
    for(int i = 0; i < num; ++i) {
        ...
        if(prob > thresh){
            ...
            detections.push_back(std::make_tuple(j, cv::Rect(left, top, right-left, bot-top), prob));
        }
    }
}

inline static void get_detections(std::vector<std::tuple<int, cv::Rect, float>>& detections, const std::vector<float>& predictions, const cv::Size& size, int classes) {
...
inline static void draw_detections(cv::Mat& image, const std::vector<std::tuple<int, cv::Rect, float>>& detections, const std::vector<std::string>& classes) {
    for (int i = 0; i < detections.size(); i++) {
        cv::rectangle(image, std::get<1>(detections[i]), cv::Scalar(255, 0, 0), 3);
        cv::putText(image, classes[std::get<0>(detections[i])] + " - " + std::to_string((int) (std::get<2>(detections[i])*100)) + "%", std::get<1>(detections[i]).tl(), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 2.0);
     }
}
